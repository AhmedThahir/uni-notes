...
$$
\begin{aligned}
\sigma^2_{{\text{eff}}_i} &= 1 + \sigma^2_{y_i} + \sigma^2_{\text{meas}_i} + \sum_{j=1}^m \left( \beta_{j} \sigma^2_{\delta_{ij}} \right)^2
\end{aligned}
$$
...
1. Run model ignoring $\sigma^2_{x_j} \forall j$
2. Use this model fit to calculate $\beta_j \forall j$ 
3. Calculate the effective variance $\sigma^2_{yi, \text{eff}}$
4. Run weighted regression with 1/effective variance (or 1/effective std)
5. Repeated steps until parameters converge (usually only takes a few (5-10) iterations)
